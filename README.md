...
services/api/            FastAPI app (source of truth, actively built)
  src/main.py             Route handlers + Pydantic models
...
```
cd services\api
docker build -t uptime-monitor-api .
docker run -d -p 3000:3000 --name uptime-api uptime-monitor-api
```

```powershell
Invoke-RestMethod -Uri http://localhost:3000/healthz -Method Get
```

Useful commands while it's running: `docker logs uptime-api`, `docker exec -it uptime-api /bin/bash`,
`docker stop uptime-api && docker rm uptime-api` to tear it down.
